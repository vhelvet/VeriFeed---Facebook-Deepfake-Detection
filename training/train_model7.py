...
Modified: Saves all models with validation accuracy >= 87%
"""
...
PATIENCE = 30
MIN_DELTA = 0.001
SAVE_THRESHOLD = 87.0  # Save models with accuracy >= this threshold

# Paths
METADATA_FILE = Path("training_data/metadata.json")
MODELS_DIR = Path("models7-1")
...
def main():
    ...
    logger.info(f"Save Threshold: Models with accuracy >= {SAVE_THRESHOLD}%")
    logger.info("="*70)
    ...
    # Track saved models and best accuracy
    saved_models = []
    best_acc = 0.0
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc >= SAVE_THRESHOLD:
            ...
            saved_models.append({'path': model_path, 'accuracy': val_acc, 'epoch': epoch+1})
            logger.info(f"💾 Model saved: {model_filename} (Acc: {val_acc:.2f}%)")
            
            # Save confusion matrix for this model
            cm_path = MODELS_DIR / f"confusion_matrix_acc_{val_acc:.2f}_epoch{epoch+1}.png"
            plot_confusion_matrix(val_labels, val_preds, cm_path)
        
        # Track best model
        if val_acc > best_acc + MIN_DELTA:
            best_acc = val_acc
            best_model_path = model_path if val_acc >= SAVE_THRESHOLD else None
            patience_counter = 0
    ...
    logger.info("\n" + "="*70)
    logger.info("SAVED MODELS SUMMARY")
    logger.info("="*70)
    logger.info(f"Total models saved (accuracy >= {SAVE_THRESHOLD}%): {len(saved_models)}")
    
    if saved_models:
        logger.info("\nSaved Models:")
        for i, model_info in enumerate(sorted(saved_models, key=lambda x: x['accuracy'], reverse=True), 1):
            logger.info(f"  {i}. {model_info['path'].name} - Acc: {model_info['accuracy']:.2f}% (Epoch {model_info['epoch']})")
    else:
        logger.info(f"⚠️ No models reached the {SAVE_THRESHOLD}% accuracy threshold")
    ...
    if best_model_path and best_model_path.exists():
        ...
        logger.info(f"Best accuracy achieved: {best_acc:.2f}%")
        logger.info(f"(Model not saved - below {SAVE_THRESHOLD}% threshold)")
