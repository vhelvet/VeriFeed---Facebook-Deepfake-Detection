...
class VeriFeedPredictor {
    ...
    setupScrollCloseListener() {
        // Define the closure function here, so it's always the same reference for removeEventListener
        this.scrollCloseHandler = () => {
            // Use requestAnimationFrame for performance on scroll events
        ...
        // Attach the listener to the entire window for scrolling
    ...
    addPredictButton(container, videoElement) {
        ...
        predictBtn.style.cssText = `
            ...
            /* Formal/Creative Gradient (Swapped for better highlight) */
            background: linear-gradient(135deg, #764ba2 0%, #667eea 100%) !important;
            color: white !important;
            border: 1px solid rgba(255, 255, 255, 0.1) !important; /* Added subtle border */
            ...
            /* Deeper shadow with metallic inset highlight */
            box-shadow: 0 3px 10px rgba(0, 0, 0, 0.2) !important;
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif !important; /* More formal font stack */
            display: inline-flex !important;
            align-items: center !important;
            transition: all 0.2s cubic-bezier(0.175, 0.885, 0.32, 1.275) !important; /* Creative transition */
        `;


        predictBtn.onmouseenter = () => {
            // Adjusted hover to use the original dark/light purple
            predictBtn.style.background =
                "linear-gradient(135deg, #6a4190 0%, #5a6fd8 100%)";
            predictBtn.style.transform = "translateY(-1px) scale(1.02)"; // Slight lift and enlarge
    ...
    async extractFramesFast(videoElement) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                // 1. Setup Canvas (remains the same)
                const canvas = document.createElement("canvas");
                // Removed { willReadFrequently: true } as it's often not needed and adds overhead
                ...
                // 2. Fixed Extraction Parameters (DO NOT CHANGE)
                const EXTRACT_DURATION = Math.min(duration, this.EXTRACT_DURATION); // Max 30s
                const TARGET_FRAMES = this.TARGET_FRAMES; // Fixed at 150
                const TARGET_FPS = this.TARGET_FPS; // Fixed at 5


                // Time step to achieve 5 FPS (1/5 = 0.2 seconds per frame)
                ...
                // 3. Generate all target timestamps
                const targetTimes = [];
                for (let i = 0; i < TARGET_FRAMES; i++) {
                    let time = i * timeStep;
                    if (time >= EXTRACT_DURATION) {
                        break; // Stop at EXTRACT_DURATION boundary
                    }
                    targetTimes.push(time);
                }


                // 4. Time-out Safety Mechanism
                ...
                }, 5000); // 5-second hard limit for extraction
                ...
                // 5. The Fast Extraction Loop
                for (const time of targetTimes) {
                    // Check if timeout has already resolved or if enough frames are captured
                    if (hasResolved || frames.length >= TARGET_FRAMES) break;


                    // Await a Promise that resolves when the seek operation is complete
                    await new Promise((innerResolve) => {
                        ...
                        // Set the new time to trigger the seek operation
                        videoElement.currentTime = time;
                    });


                    // Capture the frame immediately after seeking is complete
                    // Use 0.7 for higher speed / lower payload (optimized for speed/network)
    ...
    showResultsPopup(buttonElement, result) {
        ...
        const processingInfo = result.processing_time
            ? `<div class="processing-time">⚡ Analyzed in ${result.processing_time.total}s</div>`
            : "";
            ...
            <div class="verifeed-popup-content">
                ...
                <div class="verifeed-popup-body">
                    <div class="confidence-section">
                        <span class="confidence-label">Confidence: ${confidence.toFixed(
                            1
                        )}%</span>
                    ...
                    <div class="probability-section">
                        <div class="prob-item">
                            <span class="prob-label">Real: ${realProb.toFixed(
                                1
                            )}%</span>
                            <div class="prob-bar">
                        ...
                        <div class="prob-item">
                            <span class="prob-label">Fake: ${fakeProb.toFixed(
                                1
                            )}%</span>
                            <div class="prob-bar">
        ...
        resultsPopup.style.cssText = `
            ...
            box-shadow: 0 10px 40px rgba(0, 0, 0, 0.3), 0 0 0 1px #e5e7eb !important;
            animation: slideDown 0.2s ease-out !important;
        `;


        // FIX: style variable must be declared before use
        ...
        style.textContent = `
            ...
            .verifeed-popup-header {
                ...
                border-bottom: 1px solid #e5e7eb; /* Formal line separation */
            }
            .verifeed-popup-header .status-icon { font-size: 20px; margin-right: 10px; } /* Larger icon */
            .verifeed-popup-header .status-text { font-weight: 700; font-size: 15px; flex: 1; } /* Bolder, larger text */
            ...
            .probability-section {
                margin-bottom: 16px;
                padding-bottom: 16px;
                border-bottom: 1px dashed #f3f4f6; /* Creative dashed separator */
            }
            ...
            .info-text {
                ...
                padding: 14px; background: #f9fafb; border-left: 4px solid ${statusColor};
                border-radius: 4px;
                margin-bottom: 12px;
                font-style: italic;
            }
        ...
        const closePopup = () => {
            ...
            this.activePopup = null;
            this.activeStyle = null;
            window.removeEventListener('scroll', this.scrollCloseHandler); // Ensure removal
            this.scrollCloseHandler = null;
        };
        ...
        // NEW: Add scroll-to-close functionality
    ...
    showErrorPopup(buttonElement, message) {
        ...
        errorPopup.style.cssText = `
            ...
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif !important;
            background: white !important;
            border-radius: 6px !important; /* Formal radius */
            box-shadow: 0 8px 32px rgba(0, 0, 0, 0.15) !important;
            border: 2px solid #fca5a5 !important; /* Slightly lighter red border for cleaner look */
        `;
        ...
        style.textContent = `
            ...
            .verifeed-error-popup .retry-btn {
                background: linear-gradient(135deg, #764ba2 0%, #667eea 100%); /* Use the main gradient */
                color: white;
                border: none;
                padding: 8px 16px;
                border-radius: 4px;
                ...
                cursor: pointer;
                font-weight: 600;
                box-shadow: 0 2px 5px rgba(102, 126, 234, 0.3); /* Button shadow */
            }
            .verifeed-error-popup .retry-btn:hover {
                background: linear-gradient(135deg, #6a4190 0%, #5a6fd8 100%); /* Darken gradient on hover */
            }
        `;
    ...
    removeExistingPopup() {
        ...
        // NEW: Clean up the scroll listener
