...
class VeriFeedPredictor {
  ...
  async checkServerHealth() {
    try {
      ...
      if (data.status === "healthy" && data.model_loaded) {
        console.log("âœ… Backend server ready (SECURED + OPTIMIZED)");
        ...
        console.warn("âš ï¸ Backend online but model not loaded");
      }
    } catch (error) {
      console.error("âŒ Backend server offline");
    }
  }
      ...
      try {
        ...
        timeoutId = setTimeout(() => {
          if (!hasResolved) {
            ...
            console.warn(`â±ï¸ Timeout - captured ${frames.length} frames`);
            resolve(frames);
          }
        }, 5000);
        ...
        console.log(
          `âœ… Captured ${frames.length} frames in ${elapsed}s (Target: <5s)`
        );
  ...
  showResultsPopup(buttonElement, result) {
    ...
    const bgGradient = "linear-gradient(90deg, #667eea 0%, #764ba2 25%, #f093fb 50%, #4facfe 75%, #667eea 100%)";
    const bgGradientSize = "200% 100%";
    ...
    resultsPopup.innerHTML = `
            <div class="verifeed-popup-content">
                <div class="verifeed-popup-header" style="background: ${bgGradient} !important; background-size: ${bgGradientSize} !important; animation: gradientMove 5s linear infinite !important;">
                    <div class="status-indicator">
                        <span class="status-icon-animated">${statusIcon}</span>
                        <div class="status-info">
                            <span class="status-text" style="color: white !important;">${statusText}</span>
...
function initializeVeriFeed() {
  if (window.location.hostname.includes("facebook.com") && !veriFeedInstance) {
    console.log("ðŸ”® Initializing VeriFeed Predictor (SECURED + OPTIMIZED)...");
...
window.checkServer = async function () {
  if (veriFeedInstance) {
    ...
    console.log("âŒ VeriFeed not initialized");
  }
};
...
window.forceRescan = function () {
  if (veriFeedInstance) {
    console.log("ðŸ”„ Forcing video scan...");
    veriFeedInstance.scanForVideos();
  } else {
    console.log("âŒ VeriFeed not initialized");
  }
...
window.testPrediction = async function () {
  console.log("ðŸ§ª Testing prediction on first video...");
  const video = document.querySelector("video");
  if (!video) {
    console.log("âŒ No video found on page");
    return;
  }
  ...
  if (data && data.button) {
    ...
    console.log("âŒ Video not tracked by VeriFeed");
  }
};

console.log("=== VERIFEED PREDICTION MODE (SECURED + OPTIMIZED) ===");
console.log("ðŸ”’ Authentication: JWT Token with auto-refresh");
console.log("ðŸš€ Optimizations: Fast frame extraction (seek-based)");
console.log("ðŸ“Š Configuration: 150 frames at 5fps from first 30 seconds");
console.log("âš¡ Performance Features:");
console.log("   â€¢ Seek-based extraction (3-5x faster)");
console.log("   â€¢ Lower JPEG quality (0.7 for network speed)");
console.log("   â€¢ Smart frame sampling");
console.log("   â€¢ Cached face detection");
console.log("   â€¢ Mixed precision inference (CUDA)");
console.log("ðŸ”— Connects to secure backend at http://localhost:5000");
console.log("");
