...
# -------------------- API ENDPOINTS --------------------
# -------------------- API ENDPOINT WITH CORRECTED FLOW --------------------
@app.route('/frame_analyze', methods=['POST'])
def analyze_frames():
    """
    CORRECTED FLOW:
    1. Detect faces across ALL frames first
    2. Count total faces detected
    3. Select best model based on face count
    4. Process frames with selected model's sequence length
    """
    ...
    try:
        ...
        # =================================================================
        # STEP 1: DETECT FACES ACROSS ALL FRAMES (COUNT THEM FIRST!)
        # =================================================================
        logger.info("=" * 70)
        logger.info("STEP 1: 👤 DETECTING FACES IN VIDEO...")
        logger.info("=" * 70)
        ...
        frames_with_faces = 0
        frames_to_scan = min(len(frames), MAX_FRAMES_TO_PROCESS)
        
        logger.info(f"Scanning {frames_to_scan} frames for face detection...")
        ...
        for i in range(frames_to_scan):
            ...
            try:
                ...
                # Progress logging
                if (i + 1) % 20 == 0:
                    logger.info(f"  Scanned {i + 1}/{frames_to_scan} frames... "
                              f"Found {total_faces_detected} faces in {frames_with_faces} frames")
                    
            except Exception as e:
                logger.debug(f"Face detection error on frame {i}: {e}")
                continue
        ...
        logger.info(f"   • Frames with faces: {frames_with_faces}/{frames_to_scan}")
        logger.info(f"   • Detection time: {detection_time:.2f}s")
        logger.info("=" * 70)
        
        # Use the face count for model selection (minimum 10 for safety)
        faces_for_selection = max(total_faces_detected, MIN_FRAMES_REQUIRED)
        
        # =================================================================
        # STEP 2: SELECT BEST MODEL BASED ON FACE COUNT
        # =================================================================
        logger.info("\nSTEP 2: 🤖 SELECTING OPTIMAL MODEL...")
        logger.info("=" * 70)
        
        model_path, expected_seq_len = get_best_model(faces_for_selection)
        
        logger.info(f"✅ Model selected: {os.path.basename(model_path)}")
        logger.info(f"   • Expected sequence length: {expected_seq_len} frames")
        logger.info(f"   • Based on: {total_faces_detected} faces detected")
        logger.info("=" * 70)
        ...
        # =================================================================
        # STEP 4: PROCESS FRAMES WITH MODEL'S SEQUENCE LENGTH
        # =================================================================
        ...
        logger.info(f"   • Frames collected: {frames_collected}")
        logger.info(f"   • Faces extracted: {faces_detected}")
        logger.info("=" * 70)
        ...
        logger.info(f"✅ Inference complete ({inference_time:.2f}s)")
        
        # =================================================================
        # STEP 6: APPLY EXPRESSION ANALYSIS ADJUSTMENTS
        # =================================================================
        adjustments = []
        if expression_metrics:
            logger.info("\nSTEP 6: 🎭 ANALYZING EXPRESSIONS...")
            pred_idx, confidence, probs, adjustments = adjust_prediction_for_expressions(
                pred_idx, confidence, np.array(probs), 
                expression_metrics, flow_metrics, landmark_metrics
            )
            logger.info(f"✅ Expression analysis complete ({len(adjustments)} adjustments)")
        ...
        response = {
            ...
            'model_selection_basis': f"{total_faces_detected} total faces detected across video",
            'model_expected_sequence_length': expected_seq_len,
            'total_faces_in_video': total_faces_detected,
            'frames_with_faces': frames_with_faces,
            'faces_in_processed_frames': faces_detected,
            'frames_collected': frames_collected,
        ...
        print(f"🎭 Selection Basis:      {total_faces_detected} total faces detected")
        print(f"📦 Model Sequence:       {expected_seq_len} frames")
        print(f"📥 Frames Received:      {len(frames)}")
        print(f"👤 Total Faces Found:    {total_faces_detected} (in {frames_with_faces} frames)")
        print(f"🎬 Frames Processed:     {frames_collected}")
        print(f"👤 Faces Extracted:      {faces_detected}")
        ...
        if adjustments:
            print(f"🎭 Adjustments:          {len(adjustments)} expression-based adjustments")
...
def parse_model_info(model_path):
    """
    Extracts accuracy (%) and frame count (sequence length) from model filename.
    Examples:
      - model_84_acc_10_frames_final_data.pt → (84, 10)
      - model_97_acc_60_frames_FF_data.pt → (97, 60)
      - model_93_acc_100_frames_celeb_FF_data.pt → (93, 100)
    """
    filename = os.path.basename(model_path).lower()
    match = re.search(r"model_(\d+)_acc_(\d+)_frames", filename)
    
    if match:
        accuracy = int(match.group(1))
        seq_len = int(match.group(2))
        logger.debug(f"Parsed {filename}: acc={accuracy}%, seq={seq_len}")
        return accuracy, seq_len
    else:
        logger.warning(f"⚠️ Could not parse model info from: {filename}")
        return 0, SEQUENCE_LENGTH  # fallback


def get_best_model(num_faces_detected):
    """
    Select the best model based on number of faces detected.
    Strategy:
      1. Try to find EXACT match for frame count
      2. If no exact match, find CLOSEST match (prefer higher frame counts)
      3. Tie-breaker: highest accuracy
    
    Example:
      - 40 faces detected → select model with 40 frames
      - 45 faces but only 40/60 frame models → select 60 frame model
    """
    ...
    logger.info(f"\n📋 Available models (for {num_faces_detected} faces):")
    for info in sorted(model_info, key=lambda x: x['seq_len']):
        logger.info(f"   • {info['name']} → {info['seq_len']} frames, {info['accuracy']}% acc")

    # Strategy 1: Find EXACT match
    exact_matches = [m for m in model_info if m['seq_len'] == num_faces_detected]
    
    if exact_matches:
        # Multiple exact matches → pick highest accuracy
        best_model = max(exact_matches, key=lambda x: x['accuracy'])
        logger.info(f"\n✅ EXACT MATCH FOUND!")
        logger.info(f"   Selected: {best_model['name']}")
        logger.info(f"   Reason: Exact match for {num_faces_detected} faces")
        return best_model['path'], best_model['seq_len']

    # Strategy 2: Find CLOSEST match
    logger.info(f"\n⚠️  No exact match for {num_faces_detected} frames")
    logger.info(f"   Finding closest match...")
    
    # Sort by: distance to target, then prefer higher sequences, then accuracy
    closest_models = sorted(
        model_info,
        key=lambda x: (
            abs(x['seq_len'] - num_faces_detected),  # Closest distance
            -(x['seq_len'] >= num_faces_detected),   # Prefer >= target
            -x['accuracy']                            # Highest accuracy
        )
    )

    best_model = closest_models[0]
    
    if best_model['seq_len'] >= num_faces_detected:
        logger.info(f"✅ CLOSEST HIGHER MATCH")
        logger.info(f"   Selected: {best_model['name']}")
        logger.info(f"   Model has {best_model['seq_len']} frames for {num_faces_detected} faces")
        logger.info(f"   Note: Will use first {num_faces_detected} frames")
    else:
        logger.warning(f"⚠️  CLOSEST LOWER MATCH")
        logger.warning(f"   Selected: {best_model['name']}")
        logger.warning(f"   Model has {best_model['seq_len']} frames for {num_faces_detected} faces")
        logger.warning(f"   Note: Will pad extra {num_faces_detected - best_model['seq_len']} frames")

    logger.info(f"\n🎯 FINAL SELECTION:")
    logger.info(f"   Model: {best_model['name']}")
    logger.info(f"   Accuracy: {best_model['accuracy']}%")
    logger.info(f"   Sequence: {best_model['seq_len']} frames")
    logger.info(f"   Faces: {num_faces_detected}\n")
