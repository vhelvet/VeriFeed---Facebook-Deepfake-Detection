...
def predict_with_visualization(model, img_tensor, out_dir=DETECTED_FACES_DIR):
    ...
    with torch.no_grad():
        ...
        try:
            ...
            # IMPROVED: Generate CAM for ALL feature maps, not just predicted class
            cam_maps = []
            for class_idx in [0, 1]:  # Generate for both FAKE and REAL
                ...
                # IMPROVED: Better normalization to spread attention
                ...
                # IMPROVED: Apply gamma correction to enhance mid-range values
                # This makes more of the face "light up" instead of just peaks
            ...
            # IMPROVED: Apply adaptive histogram equalization for better distribution
            predict_img = np.uint8(255 * predict_img)
            
            # Alternative: Use adaptive equalization
            # from skimage import exposure
            # predict_img = exposure.equalize_adapthist(predict_img, clip_limit=0.03)
            # predict_img = np.uint8(255 * predict_img)
            ...
            # IMPROVED: Adjust blending for better visibility
            # Reduce heatmap intensity, increase image visibility
            result = heatmap * 0.4 + image * 0.8 * 255
            result = np.clip(result, 0, 255).astype(np.uint8)
            
            # Create multi-view visualization
            # Show: Original | Fake CAM | Real CAM | Final Prediction
            ...
            fake_heatmap = cv2.applyColorMap(fake_cam_resized, cv2.COLORMAP_JET)
            
            # Real class CAM
            real_cam = np.uint8(255 * cam_maps[1])
            real_cam_resized = cv2.resize(real_cam, (im_size, im_size))
            real_heatmap = cv2.applyColorMap(real_cam_resized, cv2.COLORMAP_JET)
            ...
            # Stack horizontally: Original | Fake CAM | Real CAM | Prediction
            ...
            visualization_path = multi_path  # Return the comprehensive view
            visualization_b64 = vis_b64
            
            # Calculate attention coverage score
            # Measure how much of the face the model is actually looking at
            attention_threshold = 0.3  # Consider pixels with >30% activation as "attended"
...
def get_best_model(num_faces_detected):
    ...
    # FIXED LOGIC: Different selection criteria based on face count
    if num_faces_detected <= 30:
        # For few/no faces: prioritize SHORTER sequences and higher accuracy
        # Shorter sequences avoid overfitting and are faster
...
def analyze_frames():
    ...
    try:
        ...
        response = {
            ...
            'bias_warning': bias_warning,  # NEW: Alert for low-confidence REAL predictions
            'raw_prediction': int(pred_idx),
            'all_probabilities': {
                'FAKE': round(probs[0] * 100, 2),
                'REAL': round(probs[1] * 100, 2)
            },
            'probability_difference': round(abs(probs[1] - probs[0]) * 100, 2),  # NEW: Show margin
...
if __name__ == '__main__':
    ...
    print("\n✨ MODEL SELECTION:")
    print("   • Prioritizes longer sequences (better temporal analysis)")
    print("   • Longer sequences = better fake detection capability")
    print("   • Note: Model may have bias toward REAL - check confidence margins")
